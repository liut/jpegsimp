...
func TestJPEG(t *testing.T) {
	...
	cr := new(CountWriter)
	im, err := Open(io.TeeReader(rd, cr))
	assert.NoError(t, err)
	assert.NotNil(t, im)
	assert.Equal(t, int(jpegSize), cr.Len())
	attr := im.GetAttr()
	assert.NotNil(t, attr)
	assert.Equal(t, jpegWidth, attr.Width)
	assert.Equal(t, jpegHeight, attr.Height)
	// assert.Equal(t, ".jpg", attr.Ext)
	t.Logf("quality: %d", attr.Quality)
	assert.Equal(t, jpegQuality, attr.Quality)

	var buf bytes.Buffer
	im.SetOption(WriteOption{Quality: 84})
	err = im.WriteTo(&buf)
	assert.NoError(t, err)
	assert.NotZero(t, buf.Len())

	assert.Equal(t, 5266, buf.Len())
}
...
func TestOptimize(t *testing.T) {
	...
	si, so, err := Optimize(rd, &buf, &WriteOption{Quality: 78})
	assert.NoError(t, err)
	assert.NotZero(t, si)
	assert.NotZero(t, so)
